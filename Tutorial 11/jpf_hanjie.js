...
function init() {
   ...
   // Add an event listener to the Show Solution button (Anonymous function example)
   document.getElementById("solve").addEventListener("click", 
      function() {
         // Remove the inline backgroundColor style from each cell
         for (var i = 0; i < puzzleCells.length; i++) {
            puzzleCells[i].style.backgroundColor = ""; // Erases previous inline style, leaving only CSS sylesheet styles applied 
         }
      }
   );
}
...
function swapPuzzle(e) {
   ...
   // Alert user that they will lose progress on current puzzle if they swap for another one
   if (confirm("You will lose all of your work on the puzzle! Continue?")) {

      // Get the ID of the event target, the button where the click occurred 
      var puzzleID = e.target.id;

      // Extract the title of the puzzle from value attribute in the puzzle buttons 
      var puzzleTitle = e.target.value;
      document.getElementById("puzzleTitle").innerHTML = puzzleTitle;

      // Switch statement that calls the drawPuzzle() function for each of the three 
      // possible puzzle ID values
      switch (puzzleID) {
         case "puzzle1":
...
function setupPuzzle() {
   ...
   // Create an event listener to highlight incorrect cells (PEEK FUNCTION)
